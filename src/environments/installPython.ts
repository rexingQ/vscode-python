...
export function activate(context: ExtensionContext) {
    context.subscriptions.push(commands.registerCommand('python.envManager.installPython', async () => {
        if (await fs.pathExists(MICROMAMBA_EXE)) {
            ...
            `If it does not worker, then initiliaze your shell using the command '${getDisplayPath(MICROMAMBA_EXE)} shell init -s bash|zsh|cmd.exe|powershell|fish|xonsh -p ~/micromamba'.`];
...
async function createInstallContext() {
    const installed = await fs.pathExists(MICROMAMBA_EXE);
    void commands.executeCommand('setContext', 'python.envManager.pythonIsNotInstalled', !installed);
}
